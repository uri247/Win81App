...
void DeviceConnect::OnNavigatedTo(NavigationEventArgs^ /* eventArgs */)
{
    ...
    if (EventHandlerForDevice::Current->IsDeviceConnected 
        ...
        && EventHandlerForDevice::Current->DeviceInformation != nullptr))
    {
        ...
        EventHandlerForDevice::Current->OnDeviceConnected = ref new TypedEventHandler<EventHandlerForDevice^, DeviceInformation^>(this, &DeviceConnect::OnDeviceConnected);
...
void DeviceConnect::OnNavigatedFrom(NavigationEventArgs^ /* eventArgs */)
{
    StopDeviceWatchers();
...
void DeviceConnect::ConnectToDevice_Click(Object^ /* sender */, RoutedEventArgs^ /* eventArgs */)
{
    ...
    if (selection->Size > 0)
    {
        ...
        entry = static_cast<DeviceListEntry^>(obj);
...
/// </summary>
void DeviceConnect::InitializeOsrFx2DeviceWatcher(void)
{
    auto osrFx2Selector = UsbDevice::GetDeviceSelector(OsrFx2::DeviceVid, OsrFx2::DevicePid);

    // Create a device watcher to look for instances of the OSRFX2 device interface
    auto osrFx2Watcher = DeviceInformation::CreateWatcher(osrFx2Selector);
...
/// Please see IntializeDeviceWatchers for more information on using device selectors
/// </summary>
void DeviceConnect::InitializeSuperMuttDeviceWatcher(void)
{
    auto superMuttSelector = UsbDevice::GetDeviceSelector(SuperMutt::DeviceVid, SuperMutt::DevicePid, SuperMutt::DeviceInterfaceClass);
...
/// <param name="sender"></param>
/// <param name="deviceInformation"></param>
void DeviceConnect::OnDeviceConnected(EventHandlerForDevice^ /* sender */, DeviceInformation^ /* deviceInformation */)
{
