...
{
    return create_async([this, deviceInfo, deviceSelector]()
    {
        return create_task(UsbDevice::FromIdAsync(deviceInfo->Id))
            .then([this, deviceInfo, deviceSelector](task<UsbDevice^> deviceTask) 
            {
                ...
                if (device != nullptr)
                {
                    ...
                    if (!isBackgroundTask && (appSuspendEventToken.Value == 0 || appResumeEventToken.Value == 0))
                    {
