...
Namespace Global.SDKTemplate
    ...
    ''' </summary>
    Partial Public NotInheritable Class FileOpenPickerPage
        ...
        Friend fileOpenPickerUI As FileOpenPickerUI = Nothing

        Private scenariosList As New List(Of Scenario)() From { _
            New Scenario() With {.Title = "Pick a file from the application package", .ClassType = GetType(FilePickerContracts.FileOpenPicker_PickAppFile)}, _
            New Scenario() With {.Title = "Pick a file from a URI", .ClassType = GetType(FilePickerContracts.FileOpenPicker_PickURLFile)}, _
            New Scenario() With {.Title = "Pick cached file", .ClassType = GetType(FilePickerContracts.FileOpenPicker_PickCachedFile)} _
        }
        ...
        Public Sub New()
            ...
            AddHandler SizeChanged, AddressOf FileOpenPickerPage_SizeChanged
        End Sub

        Private Sub FileOpenPickerPage_SizeChanged(ByVal sender As Object, ByVal e As SizeChangedEventArgs)
            InvalidateSize()
            If FileOpenPickerPageResizedEvent IsNot Nothing Then
                Dim args As New FileOpenPickerPageSizeChangedEventArgs()
                args.Width = Me.ActualWidth
                RaiseEvent FileOpenPickerPageResized(Me, args)
            End If
        End Sub
