...
Namespace Global.SDKTemplate
    ...
    ''' </summary>
    Public NotInheritable Partial Class CachedFileUpdaterPage
        Inherits Global.SDKTemplate.Common.LayoutAwarePage

        Public Const FEATURE_NAME As String = "Cached File Updater Page"
        Public Event ScenarioLoaded As System.EventHandler
        Public Event CachedFileUpdaterPageResized As EventHandler(Of CachedFileUpdaterPageSizeChangedEventArgs)

        Public LaunchArgs As Windows.ApplicationModel.Activation.LaunchActivatedEventArgs

        Public Shared Current As CachedFileUpdaterPage

        Friend cachedFileUpdaterUI As CachedFileUpdaterUI = Nothing
        Friend fileUpdateRequest As FileUpdateRequest = Nothing
        Friend fileUpdateRequestDeferral As FileUpdateRequestDeferral = Nothing

        Private dispatch As CoreDispatcher = Window.Current.Dispatcher

        Private HiddenFrame As Frame = Nothing

        Private scenariosList As List(Of Scenario) = Nothing

        Public Sub Activate(ByVal args As CachedFileUpdaterActivatedEventArgs)
            ' cache CachedFileUpdaterUI
            cachedFileUpdaterUI = args.CachedFileUpdaterUI

            AddHandler cachedFileUpdaterUI.FileUpdateRequested, AddressOf CachedFileUpdaterUI_FileUpdateRequested
            AddHandler cachedFileUpdaterUI.UIRequested, AddressOf CachedFileUpdaterUI_UIRequested

            Select Case cachedFileUpdaterUI.UpdateTarget
                Case CachedFileTarget.Local
                    scenariosList = New List(Of Scenario) From {New Scenario() With {.Title = "Get latest version", .ClassType = GetType(FilePickerContracts.CachedFileUpdater_Local)}}
                Case CachedFileTarget.Remote
                    scenariosList = New List(Of Scenario) From {New Scenario() With {.Title = "Remote file update", .ClassType = GetType(FilePickerContracts.CachedFileUpdater_Remote)}}
            End Select

            Window.Current.Activate()
        End Sub

        Private Async Sub CachedFileUpdaterUI_UIRequested(ByVal sender As CachedFileUpdaterUI, ByVal args As Object)
        ...
        Private Sub CachedFileUpdaterUI_FileUpdateRequested(ByVal sender As CachedFileUpdaterUI, ByVal args As FileUpdateRequestedEventArgs)
            fileUpdateRequest = args.Request
            fileUpdateRequestDeferral = fileUpdateRequest.GetDeferral()
            Select Case cachedFileUpdaterUI.UIStatus
        ...
        Public Sub New()
            ...
            ' Populate the sample title from the constant in the GlobalVariables.vb file.
            SetFeatureName(FEATURE_NAME)

            AddHandler scenarios.SelectionChanged, AddressOf Scenarios_SelectionChanged
            AddHandler SizeChanged, AddressOf CachedFileUpdaterPage_SizeChanged
        End Sub

        Private Sub CachedFileUpdaterPage_SizeChanged(ByVal sender As Object, ByVal e As SizeChangedEventArgs)
            InvalidateSize()
            If CachedFileUpdaterPageResizedEvent IsNot Nothing Then
                Dim args As New CachedFileUpdaterPageSizeChangedEventArgs()
                args.Width = Me.ActualWidth
                RaiseEvent CachedFileUpdaterPageResized(Me, args)
            End If
        End Sub
    ...
    Public Class CachedFileUpdaterPageSizeChangedEventArgs
        Inherits EventArgs
