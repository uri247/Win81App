...
void ReadMessage::DoReadMessage()
{
    ...
    if ((getId!=nullptr) && (getId->Value >= 1) && (getId->Value<=UINT32_MAX) && (getId->Value <= device->MessageStore->MaxMessages))
    {
        ...
        }).then([this] (ISmsMessage^ msg)
        {
            ...
            if (textMsg != nullptr)
            {                
                // Set up DateTimeFormatter to display the full time and date of the text message.
                DateTimeFormatter^ dateFormatter = ref new DateTimeFormatter("longdate");
                DateTimeFormatter^ timeFormatter = ref new DateTimeFormatter("longtime");
                DateText->Text = timeFormatter->Format(textMsg->Timestamp) + " " + dateFormatter->Format(textMsg->Timestamp);
