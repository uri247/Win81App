...
Partial Public NotInheritable Class ImagingTransforms
    ...
    Private Const WINCODEC_ERR_UNSUPPORTEDOPERATION As Integer = CInt(&H88982F81)
    ' This file format does not support the requested property/metadata query.
    Private Const WINCODEC_ERR_PROPERTYNOTSUPPORTED As Integer = CInt(&H88982F41)
    ' There is no codec or component that can handle the requested operation; for example, encoding.
    Private Const WINCODEC_ERR_COMPONENTNOTFOUND As Integer = CInt(&H88982F50)
    ...
    ''' This method does not use the SuspensionManager helper class (defined in SuspensionManager.vb).
    ''' </summary>
    Private Sub SaveDataToPersistedState(ByVal sender As Object, ByVal args As SuspendingEventArgs)
        ' Only save state if we have valid data.
        If m_fileToken IsNot Nothing Then
    ...
    Private Async Sub RestoreDataFromPersistedState()
        Try
            ...
            m_fileToken = CStr(m_localSettings("scenario2FileToken"))
            ...
            Do While desiredOrientation <> m_userRotation
    ...
    ''' </summary>
    Private Async Sub Open_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        ResetPersistedState()
        ResetSessionState()
    ...
    ''' </summary>
    Private Async Function DisplayImageFileAsync(ByVal file As StorageFile) As Task
    ...
    ''' <returns></returns>
    Private Async Function GetImageInformationAsync(ByVal file As StorageFile) As Task
        Using stream As IRandomAccessStream = Await file.OpenAsync(FileAccessMode.Read)
            ...
            Catch err As Exception
                Select Case err.HResult
                    ...
                    Case WINCODEC_ERR_UNSUPPORTEDOPERATION, WINCODEC_ERR_PROPERTYNOTSUPPORTED
    ...
    ''' </summary>
    Private Async Sub Save_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
        ...
        Catch err As Exception
            If err.HResult = WINCODEC_ERR_COMPONENTNOTFOUND Then
    ...
    ''' </summary>
    Private Async Sub SaveAs_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            Using inputStream As IRandomAccessStream = Await inputFile.OpenAsync(FileAccessMode.Read), outputStream As IRandomAccessStream = Await outputFile.OpenAsync(FileAccessMode.ReadWrite)
                ...
                ' Scaling occurs before flip/rotation, therefore use the original dimensions
                ' (no orientation applied) as parameters for scaling.
                transform.ScaledHeight = CUInt(decoder.PixelHeight * m_scaleFactor)
                transform.ScaledWidth = CUInt(decoder.PixelWidth * m_scaleFactor)
                ...
                encoder.SetPixelData(format, alpha, CUInt(CDbl(m_displayWidthNonScaled) * m_scaleFactor), CUInt(CDbl(m_displayHeightNonScaled) * m_scaleFactor), decoder.DpiX, decoder.DpiY, pixels)
    ...
    Private Sub ScaleSlider_ValueChanged(ByVal sender As Object, ByVal e As Windows.UI.Xaml.Controls.Primitives.RangeBaseValueChangedEventArgs)
        ' Ignore any value changed events the control is set up and we are accepting user input.
        If (ScaleSlider IsNot Nothing) AndAlso (ScaleSlider.IsEnabled = True) Then
    ...
    ''' </summary>
    Private Sub UpdateImageRotation(ByVal rotation As PhotoOrientation)
    ...
    Private Async Sub ResetSessionState()
        ...
        Using stream As IRandomAccessStream = Await placeholderImage.OpenAsync(FileAccessMode.Read)
