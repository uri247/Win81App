...
String^ Util::PrintExceptionCode(String^ message, int error)
{
    ...
    if (message)
    {
        hr = StringCchPrintf(buf, ARRAYSIZE(buf), L"Unexpected exception occured:%s, error number: 0x%x", message->Data(), error);
    }
    else
    {
        hr = StringCchPrintf(buf, ARRAYSIZE(buf), L"Unexpected exception occured, error number: 0x%x", error);
    }
