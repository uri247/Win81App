...
namespace PdfShowcase
{
	...
	void Renderer::GetD2DDevice(_Outptr_ ID2D1Device** ld2dDevice)
	{
		*ld2dDevice = ComPtr<ID2D1Device>(d2dDevice).Detach();
	}

    void Renderer::GetD2DDeviceContext(_Outptr_ ID2D1DeviceContext** ld2dDeviceContext)
    {
        *ld2dDeviceContext = ComPtr<ID2D1DeviceContext>(d2dDeviceContext).Detach();
    }

    void Renderer::GetDXGIDevice(_Outptr_ IDXGIDevice** ldxgiDevice)
    {
        *ldxgiDevice = ComPtr<IDXGIDevice>(dxgiDevice).Detach();
    }

    void Renderer::GetPdfNativeRenderer(_Outptr_ IPdfRendererNative** nativePdfRenderer)
    {
        *nativePdfRenderer = ComPtr<IPdfRendererNative>(pdfRenderer).Detach();
    }
}