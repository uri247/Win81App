...
{
    InitializeComponent();
    this->Suspending += ref new SuspendingEventHandler(this, &AccountPictureName::App::OnSuspending);
}
...
void App::SetupWindowing(Windows::ApplicationModel::Activation::LaunchActivatedEventArgs^ pArgs)
{
    if (Window::Current->Content == nullptr)
    {
        ...
        TypeName pageType = { "AccountPictureName.MainPage", TypeKind::Custom };
...
void App::OnActivated(IActivatedEventArgs^ args)
{
    ...
    if (args->Kind == ActivationKind::Protocol)
    {
        ...
        if (protocolArgs->Uri->SchemeName->Equals("ms-accountpictureprovider"))
        {
            ...
            rootPage->ProtocolEvent = protocolArgs;
            MainPage::Current->LoadSetAccountPictureScenario();
        }
    }
    Window::Current->Activate();
}
