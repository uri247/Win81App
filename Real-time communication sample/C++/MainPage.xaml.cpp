    ...
    , _dispatcher(Window::Current->Dispatcher)
{
    ...
    _mediaExtensionMgr = ref new Windows::Media::MediaExtensionManager();
    _mediaExtensionMgr->RegisterSchemeHandler("Microsoft.Samples.SimpleCommunication.StspSchemeHandler", "stsp:");
...
void MainPage::Scenarios_SelectionChanged(Object^ sender, SelectionChangedEventArgs^ e)
{
    if (Scenarios->SelectedItem != nullptr)
    {
        ...
        long scenarioLocked = _InterlockedExchangeAdd(&_navigationDisabled, 0);
        if (-1 != scenarioLocked)
        {
            // Scenario locked
            Scenarios->SelectedIndex = scenarioLocked;
        }
        else
        {
...
void MainPage::LockScenarioChange()
{
    long prev = _InterlockedCompareExchange(&_navigationDisabled, (long)Scenarios->SelectedIndex, -1);
    if (prev != -1)
    {
        // Someone has locked.
        return;
...
void MainPage::UnlockScenarioChange()
{
    _InterlockedExchange(&_navigationDisabled, -1);
}

void MainPage::SetCurrentScenario(Platform::String^ scenario)
{
    _currentScenario = scenario;
}

bool MainPage::NavigatingPrevented()
{
    return _currentScenario && -1 != _InterlockedExchangeAdd(&_navigationDisabled, 0);
}
