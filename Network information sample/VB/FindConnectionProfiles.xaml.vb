...
''' </summary>
Partial Public NotInheritable Class FindConnectionProfiles
    ...
    ''' <param name="e"></param>
    Private Sub FindConnectionProfilesButton_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Dim serviceProviderGuid As Guid

        ' Add values to the filter. If we don't care about a particular field, we don't set it.
        Dim filter As New ConnectionProfileFilter()
        Select Case CType(IsConnectedComboBox.SelectedItem, ComboBoxItem).Content.ToString()
            Case "Yes"
        ...
        Select Case CType(IsWlanProfileComboBox.SelectedItem, ComboBoxItem).Content.ToString()
            Case "Yes"
        ...
        Select Case CType(IsWwanProfileComboBox.SelectedItem, ComboBoxItem).Content.ToString()
            Case "Yes"
        ...
        Select Case CType(NetworkCostTypeComboBox.SelectedItem, ComboBoxItem).Content.ToString()
            Case "Unrestricted"
        ...
        If Guid.TryParse(ServiceProviderGuidTextBox.Text, serviceProviderGuid) Then
            filter.ServiceProviderGuid = serviceProviderGuid
        End If

        NetworkInformation.FindConnectionProfilesAsync(filter).Completed = AddressOf FindConnectionProfilesCompletedHandler
    End Sub

    Private Sub FindConnectionProfilesCompletedHandler(ByVal asyncInfo As IAsyncOperation(Of IReadOnlyList(Of ConnectionProfile)), ByVal status As AsyncStatus)
        If status = AsyncStatus.Completed Then
            Dim connectionProfiles As IReadOnlyList(Of ConnectionProfile) = asyncInfo.GetResults()
            asyncInfo.Close()
    ...
    Private Sub PrintConnectionProfiles(ByVal connectionProfiles As IReadOnlyList(Of ConnectionProfile))
        Dim connectionProfileList As String = String.Empty
        Try
            For Each connectionProfile In connectionProfiles
                ...
                connectionProfileList &= "--------------------------------------------------------------------" & vbLf
            Next connectionProfile

            If connectionProfiles.Count = 0 Then
                connectionProfileList &= "No Connection Profiles found." & vbLf
            End If

            rootPage.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, Sub() rootPage.NotifyUser(connectionProfileList, NotifyType.StatusMessage))
