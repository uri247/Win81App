...
void MainPage::NavigateToContactEventPage()
{
    if (this->ContactEvent != nullptr)
    {
        ...
        {
            Scenarios->SelectedIndex = 0;
            NotifyUser("This app can't handle the contact action verb it was activated for.", NotifyType::ErrorMessage);
        }
...
void MainPage::NavigateToProtocolEventPage()
{
    if (this->ProtocolEvent != nullptr)
    {
        Scenarios->SelectedIndex = 0;
        Scenarios->ScrollIntoView(Scenarios->SelectedItem);
        LoadScenario((safe_cast<ListBoxItem^>(Scenarios->SelectedItem))->Name);
