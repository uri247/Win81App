...
MainPage::MainPage()
{
    ...
    // This frame is hidden.  It is simply used to load
    // each scenario page and extract the the input and output sections and
    ...
    // Populate the sample title constants.h
    SetSecondaryTiles(FEATURE_NAME);

    this->Loaded += ref new RoutedEventHandler(this, &MainPage::MainPage_Loaded);
...
{
    // Fire the ScenarioLoaded event so downstream pages know that the respective
    // scenario files have been loaded.
...
void MainPage::NavigateToLaunchedFromSecondaryTile()
{
    unsigned int selectedListBoxItemIndex = 0;

    for (selectedListBoxItemIndex = 0; selectedListBoxItemIndex < scenarios->Length; selectedListBoxItemIndex++)
    {
        Scenarios->SelectedIndex = selectedListBoxItemIndex;
        Platform::String^ str = (safe_cast<ListBoxItem^>(Scenarios->SelectedItem))->Name;
        if (str->Equals("SecondaryTiles.LaunchedFromSecondaryTile"))
        {
            break;
        }
    }
...
void MainPage::SetSecondaryTiles(String^ strFeature)
{
    SecondaryTiles->Text = strFeature;
}
