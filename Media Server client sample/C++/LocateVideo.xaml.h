...
namespace SDKSample
{
    namespace MediaServerClient
    {
        ...
        private:
    		...
    		void LoadMediaFiles();
            concurrency::task<int> iterateOnArrayOfFolders(IVectorView<StorageFolder^>^ folders, int folderIndex, IVector<StorageFile^>^ collectedMediaItems, int maxFilesToRetrieve);
            concurrency::task<int> browseForVideoFiles(StorageFolder^ folder, IVector<StorageFile^>^ collectedMediaItems, int maxFilesToRetrieve);
