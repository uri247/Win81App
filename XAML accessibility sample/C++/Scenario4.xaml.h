		...
		public ref class MyList sealed : Windows::UI::Xaml::Controls::ListView
		{
			protected:
				virtual void PrepareContainerForItemOverride(Windows::UI::Xaml::DependencyObject^ element, Platform::Object^ item) override
				{
                    ...
                    idBinding->Path = ref new PropertyPath("Content.AutomationId");
					idBinding->RelativeSource = ref new RelativeSource();
                    idBinding->RelativeSource->Mode = RelativeSourceMode::Self;
                    source->SetBinding(AutomationProperties::AutomationIdProperty, idBinding);
                    nameBinding->Path = ref new PropertyPath("Content.AutomationName");
                    nameBinding->RelativeSource = ref new RelativeSource();
                    nameBinding->RelativeSource->Mode = RelativeSourceMode::Self;
