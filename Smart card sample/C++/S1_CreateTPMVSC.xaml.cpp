...
#include "pch.h"
#include "S1_CreateTPMVSC.xaml.h"
...
void Scenario1::Create_Click(Platform::Object^ sender,
                             Windows::UI::Xaml::RoutedEventArgs^ e)
{
    ...
    catch(Exception^ ex)
    {
        MainPage::Current->NotifyUser(
            "Failed to parse PIN policy due to exception: " +
            ex->ToString(),
    ...
    MainPage::Current->NotifyUser("Creating TPM virtual smart card...",
                                  NotifyType::StatusMessage);

    create_task(SmartCardProvisioning::RequestVirtualSmartCardCreationAsync(
    ...
    { 
        ...
        // handler is added.
        {
            ...
            this->reader->CardAdded += ref new
                TypedEventHandler<SmartCardReader ^, CardAddedEventArgs ^>(
                    this,
                    &Scenario1::HandleCardAdded,
                    Platform::CallbackContext::Same);
        }

        MainPage::Current->SmartCardReaderDeviceId =
...
void Scenario1::HandleCardAdded(SmartCardReader^ evtReader,
                                CardAddedEventArgs^ args)
{
    MainPage::Current->NotifyUser(
        "Card added to reader " + evtReader->Name + ".",
        NotifyType::StatusMessage);
}

SmartCardPinPolicy^ Scenario1::ParsePinPolicy()
{
    SmartCardPinPolicy^ pinPolicy = ref new SmartCardPinPolicy();

    DecimalFormatter^ formatter = ref new DecimalFormatter();
    Platform::IBox<unsigned long long>^ minLength =
        formatter->ParseUInt(PinMinLength->Text);
    Platform::IBox<unsigned long long>^ maxLength =
        formatter->ParseUInt(PinMaxLength->Text);
