...
#include "pch.h"
#include "S7_ListSmartCards.xaml.h"
...
void Scenario7::ListSmartCards_Click(Platform::Object^ sender,
                                     Windows::UI::Xaml::RoutedEventArgs^ e)
{
    ...
    // cards.
    smartCardItems = 
        ref new Platform::Collections::Vector<SmartCardItem^>();
    ItemListView->ItemsSource = smartCardItems;

    MainPage::Current->NotifyUser("Enumerating smart cards...",
    ...
    [&](task<DeviceInformationCollection^> getDevicesTask)
    {
            ...
            ++deviceIndex)
        {
            ...
            create_task(SmartCardReader::FromIdAsync(
                devices->GetAt(deviceIndex)->Id)).then(
            ...
            [this](task<IVectorView<SmartCard^>^> getCardsTask)
            {
                std::vector<task<SmartCardProvisioning^>> provisioningTasks;

                IVectorView<SmartCard^>^ cards = getCardsTask.get();
                for(UINT32 cardIndex = 0;
                    cardIndex < cards->Size;
                    ++cardIndex)
                {
                    SmartCard^ card = cards->GetAt(cardIndex);

                    // 3. For each card, instantiate a SmartCardProvisioning
                    //    object using
                    //    SmartCardProvisioning::FromSmartCardAsync
                    provisioningTasks.push_back(create_task(
                        SmartCardProvisioning::FromSmartCardAsync(card)));
                }

                when_all(begin(provisioningTasks),
                         end(provisioningTasks)).then(
                [=](std::vector<SmartCardProvisioning^> provisioningVector)
                {
                    for(std::vector<SmartCardProvisioning^>::iterator it =
                        provisioningVector.begin();
                        it < provisioningVector.end();
                        ++it)
                    {
                        ...
                        [=](task<String^> getNameTask)
                        {
                            ...
                            item->ReaderName = 
                                provisioning->SmartCard->Reader->Name;
                ...
                [](task<void> checkExceptionTask)
                {
                    ...
                    try
                    {
                        ...
                        MainPage::Current->NotifyUser(
                            "Enumerating smart cards completed.",
                            NotifyType::StatusMessage);
                    }
                    catch(COMException^ ex)
                    {
