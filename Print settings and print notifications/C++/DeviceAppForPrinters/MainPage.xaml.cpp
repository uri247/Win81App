...
MainPage::MainPage()
{
    ...
    SetDeviceAppForPrinters(FEATURE_NAME);
    ...
    // Change _isDebugMode to true in order to see debug output
    // By default debug mode is turned off
    _isDebugMode = false;
    if (_isDebugMode)
    {
        MessageBlock->Visibility = Windows::UI::Xaml::Visibility::Visible;
        ScenarioListLabel->Visibility = Windows::UI::Xaml::Visibility::Visible;
        Scenarios->Visibility = Windows::UI::Xaml::Visibility::Visible;
    }
    else
    {
        MessageBlock->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
        ScenarioListLabel->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
        Scenarios->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
    }
}
...
void MainPage::SetDeviceAppForPrinters(String^ strFeature)
{
    DeviceAppForPrinters->Text = strFeature;
}

void MainPage::NotifyUser(String^ strMessage, NotifyType type)
{
    // Hide progress ring by default
    ProgressBlock->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
    ...
    case NotifyType::WaitMessage:
        // Show the progress ring
        ProgressBlock->Visibility = Windows::UI::Xaml::Visibility::Visible;
        // Fall through
...
void MainPage::AddMessage(String^ strMessage)
{
    if (_isDebugMode)
    {
        MessageBlock->Text += strMessage + "\r";
    }
}

bool MainPage::SetContext(Platform::Object^ context)
{
    bool bResult = false;
    _printerExtensionContext = context;
    if (nullptr == _printHelper)
    {
        _printHelper = ref new DeviceAppForPrinters::PrintHelper();
    }
    if (nullptr != _printHelper)
    {
        bResult = _printHelper->Initialize(_printerExtensionContext);
        AddMessage("PrintHelper Initialized=" + bResult.ToString());
    }
    return bResult;
}
