...
Direct2D supports DDS files using the following block compression formats:
...
The BlockCompressedImages Direct2D C++ project uses block compressed (DDS) images that are generated
...
2. In Solution Explorer, expand the BlockCompressedImages project, and then
   right click on the Assets filter > Add > Existing Item.

3. Navigate to the BlockCompressedAssets folder and add the two DDS images.
   (guitar-transparent.dds and oldWood4_nt.dds)
   Note that these DDS images are still located in the BlockCompressedAssets folder but are displayed
   in Solution Explorer under the Assets filter.
...
- By default, any new images that are set as Item Type = Image Content Pipeline will use
  premultiplied alpha and will not generate mipmaps. This matches the requirements of Direct2D.
