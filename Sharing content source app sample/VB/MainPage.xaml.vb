    ...
    Partial Public NotInheritable Class MainPage
        ...
        Private Sub PopulateScenarios()
            ...
            For Each s As Scenario In scenariosList
                ...
                i += 1
                s.Title = (i).ToString(CultureInfo.InvariantCulture) & ") " & s.Title
                item.Content = s
            ...
            If SuspensionManager.SessionState.ContainsKey("SelectedScenarioIndex") Then
                Dim selectedScenarioIndex As Integer = Convert.ToInt32(SuspensionManager.SessionState("SelectedScenarioIndex"), CultureInfo.InvariantCulture)
        ...
        Public Sub LoadScenario(ByVal scenarioClass As Type)
            ...
            If input Is Nothing Then
                ...
                NotifyUser(String.Format(CultureInfo.CurrentCulture, "Cannot load scenario input section for {0}.  Make sure root of input section markup has x:Name of 'Input'", scenarioClass.Name), NotifyType.ErrorMessage)
                Return
            End If

            If output Is Nothing Then
                ' Malformed output section.
                NotifyUser(String.Format(CultureInfo.CurrentCulture, "Cannot load scenario output section for {0}.  Make sure root of output section markup has x:Name of 'Output'", scenarioClass.Name), NotifyType.ErrorMessage)
                Return
            End If
            ...
            Else
                ...
                NotifyUser(String.Format(CultureInfo.CurrentCulture, "Cannot load scenario: '{0}'.  Make sure root tag in the '{0}' file has an x:Name of 'LayoutRoot'", scenarioClass.Name), NotifyType.ErrorMessage)
            End If
        End Sub
        ...
        Public Sub LoadScenarioForProtocolActivation(ByVal protocolArgs As ProtocolActivatedEventArgs)
            For Each scenario As Scenario In scenariosList
                If protocolArgs.Uri.Equals(scenario.ApplicationLink) Then
                    ' By setting the selected index, the Scenarios_SelectionChanged(...) method will be called
                    Scenarios.SelectedIndex = scenariosList.IndexOf(scenario)
                    Exit For
                End If
            Next scenario
        End Sub
